...
+ first create the `data collection spreadsheet` with `mainAllCompaniesDataCollectionSheets()`, which will be located in `parentFolderID/folderName` of `@rdresearch`'s Drive.
