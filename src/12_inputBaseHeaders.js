// --- // Top-Level Sheet Headings // --- //

/* 
global
    Styles, defineNamedRange, indexPrefix, specialRangeName, doRepairsOnly
*/

// eslint-disable-next-line no-unused-vars
function addMainSheetHeader(
    SS,
    Sheet,
    Category,
    Indicator,
...
function addIndicatorGuidance(
    ...
    let indStart = row
    let values = []
    let prefix, isNew, isRevised

    Indicator.elements.forEach((Element) => {
        isNew = Element.isNew ? true : false
        isRevised = Element.isRevised ? true : false
        prefix = isRevised ? ' (rev.)' : isNew ? ' (new)' : ''

        values.push([prefix + ' ' + Element.labelShort + ':', Element.description])
        Sheet.getRange(row, 2, 1, maxColHeadings).merge().setWrap(true).setVerticalAlignment('top')
        row += 1
    })
    ...
    if (includeRGuidanceLink) {
        ...
        Sheet.getRange(row, 2, 1, maxColHeadings).merge().setWrap(true).setValue(indicatorLink)
...
function addMainCompanyHeader(Sheet, Company, activeRow, companyNrOfServices, offSetCol) {
    ...
    Sheet.getRange(activeRow, activeCol).setValue(cellValue).setWrap(true)
    ...
    Cell = Sheet.getRange(activeRow, activeCol).setBackground('#fff2cc')
    ...
    for (let i = 0; i < companyNrOfServices; i++) {
        Sheet.getRange(activeRow, activeCol).setValue(Company.services[i].label.current).setBackground('#b7e1cd')
        activeCol += 1
    }

    let horizontalDim = 1 + 2 + companyNrOfServices
    Sheet.getRange(activeRow, 1, 1, horizontalDim).setFontWeight('bold').setVerticalAlignment('middle').setFontSize(12)
...
function addSubStepHeader(SS, Sheet, Indicator, Company, activeRow, Substep, stepCNr, companyNrOfServices) {
    ...
    Cell.setValue(Substep.labelShort).setBackground(Substep.subStepColor)
...
function addExtraInstruction(Substep, stepCNr, activeRow, activeCol, Sheet, Company, companyNrOfServices) {
    ...
    Sheet.getRange(activeRow, 1, 2, 1).setBackground(Substep.subStepColor)
    ...
    Sheet.getRange(activeRow, 1, 1, horizontalDim).setBorder(
        true,
        true,
        true,
        true,
...
function addStepResearcherRow(SS, Sheet, Indicator, Company, activeRow, MainStep, companyNrOfServices) {
    ...
    let rangeStart, Range, width, rangeName
    ...
    // TODO
    // activeRow = addMainStepHeader(Sheet, Category, Company, activeRow, SS, companyNrOfServices) // sets up header
