...
/* global
    Config, indexPrefix, defineNamedRange, styleScoringIndicatorHeader, importRangeFormula, elementScoreFormula, levelScoreFormula, aggregateScoreFormula, applyCompositeScoringLogic
*/

// eslint-disable-next-line no-unused-vars
function setScoringSheetHeader(
    activeRow,
    activeCol,
    Sheet,
    Company,
...
// eslint-disable-next-line no-unused-vars
function setScoringCompanyHeader(activeRow, activeCol, Sheet, Indicator, Category, Company, blocks) {
...
function importElementBlock(
    ...
    for (let elemNr = 0; elemNr < Elements.length; elemNr++) {
        ...
        if (blocks === 1) {
            ...
            Cell.setBackground('#FFFFFF').setFontWeight('normal').setValue(rowLabel.toString()).setWrap(true)
    ...
    Block.setBorder(null, null, true, null, null, null, 'black', SpreadsheetApp.BorderStyle.DOTTED)
...
function addChangeComment(sheetModeID, activeRow, firstCol, Sheet, subStepID, Indicator, Company) {
    console.log('Adding change row')
    ...
    console.log('Cell:' + activeRow + ',' + firstCol + '------------------------------')

    let formula = '=IF(OR('
    let cellID = defineNamedRange(
        Config.indexPrefix,
        sheetModeID,
        subStepID,
        Indicator.labelShort,
